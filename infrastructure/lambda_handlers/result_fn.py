...
def handler(event, context):
    ...
    try:
        ...
        return _response(
            503,
            {"status": "error", "reason": "error", "message": "status lookup failed"},
        )
    ...
    if item_tenant and item_tenant != "unknown" and caller_tenant != item_tenant:
        ...
        print(
            f"WARNING: cross-tenant read refused: request_id={request_id}, "
    ...
    if state in ("PENDING", "QUEUED") or state is None:
        return _response(
            202,
            {"status": state or "PENDING", "request_id": request_id},
            {"Retry-After": str(RETRY_AFTER_SECONDS)},
        )
    ...
    if state == "SUCCEEDED":
        ...
        return _response(
            200,
            {"status": "SUCCEEDED", "request_id": request_id, "output_url": output_url},
        )
