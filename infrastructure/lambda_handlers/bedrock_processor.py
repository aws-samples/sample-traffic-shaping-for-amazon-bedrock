...
def _write_terminal_with_retry(
    dynamo_service,
    *,
    request_id,
    state,
    ...
    for attempt in range(1, max_tries + 1):
        try:
            ...
            last_err = e
            print(
                f"WARNING: terminal write attempt {attempt}/{max_tries} failed: "
                f"request_id={request_id}, state={state}, error={e}"
            )
            if attempt < max_tries:
                time.sleep(0.1 * attempt)
    print(
        f"ERROR: TerminalWriteFailure — request_id={request_id}, state={state}, "
...
def send_to_dlq(
    ...
    if not DLQ_URL:
        print(
            f"WARNING: DLQ_URL not configured — dropping failed request: request_id={request_id}, correlation_id={correlation_id}, error_type={error_type}"
        )
        return
    try:
        message_body = {
        ...
        sqs_client.send_message(QueueUrl=DLQ_URL, MessageBody=json.dumps(message_body))
...
def emit_token_metrics(
    model_id, source, actual_input_tokens, actual_output_tokens, correlation_id=""
):
...
def invoke_bedrock_model(model_id, request_payload):
    ...
    try:
        ...
        if is_throttled:
            ...
            print(
                f"Bedrock invocation failed: model={model_id}, error={error_code}, message={error_message}, duration={duration_ms:.2f}ms"
            )
...
def invoke_via_client(model_id, request_payload, model_config):
    ...
    max_tokens = request_payload.get("max_tokens") or int(
        model_config.get("default_max_tokens", 1024)
    )
...
def reconcile_consumption(
    dynamo_service, model_id, allocation_id, actual_input_tokens, actual_output_tokens
):
    """
    ...
    try:
        ...
        print(
            f"Reconciled consumption to actuals: model={model_id}, sk={allocation_id}, "
...
def handler(event, context):
    ...
    print(
        f"Bedrock Processor received: request_id={event.get('request_id')}, "
    ...
    if not request_id or not tenant_id:
        ...
        sfn_client.send_task_failure(
            taskToken=task_token, error="PropagationError", cause=error_msg
        )
    ...
    if not model_id:
        ...
        sfn_client.send_task_failure(
            taskToken=task_token, error="ValidationError", cause="model_id is required"
        )
    ...
    try:
        ...
        if not request_payload and execution_arn:
            ...
            request_payload = original_input.get("request_payload") or {
                ...
                **(
                    ...
                    if "temperature" in original_input
                    else {}
                ),
            }
            # User prompts may contain PII/PHI — log only the length, never the content.
            print(
                f"Resolved payload from execution: prompt_len={len(request_payload.get('prompt', ''))}"
            )
        ...
        if not request_payload or not request_payload.get("prompt"):
            ...
            sfn_client.send_task_failure(
                taskToken=task_token, error="MissingPayloadError", cause=error_msg
            )
            return {"statusCode": 400, "error": error_msg}
        ...
        try:
            ...
            print(
                f"WARNING: could not load model_config for {model_id}: {cfg_err} — using runtime converse fallback"
            )
        ...
        if (
            bedrock_response.get("success")
            and allocation_id
            and (
                bedrock_response.get("actual_input_tokens") is not None
        ...
        emit_bedrock_latency_metric(
            model_id,
            bedrock_response.get("duration_ms", 0),
            correlation_id=correlation_id,
        )
        ...
        emit_request_metrics(
            model_id,
            source,
            bedrock_response.get("throttled", False),
            correlation_id=correlation_id,
        )
        ...
        if not bedrock_response.get("success"):
            ...
            print(
                f"Sending task failure callback: request_id={request_id}, correlation_id={correlation_id}, error={error_msg}"
            )
            sfn_client.send_task_failure(
                taskToken=task_token, error="BedrockInvocationError", cause=error_msg
            )
            return {"statusCode": 502, "error": error_msg, "request_id": request_id}
        ...
        if OUTPUT_BUCKET:
            ...
            try:
                ...
                print(
                    f"ERROR: TerminalWriteFailure — failed to persist body to S3: "
            ...
            print(
                f"WARNING: OUTPUT_BUCKET unset — SUCCEEDED terminal for request_id={request_id} will have no output_ref"
            )
        ...
        if not committed:
            ...
            print(
                f"ERROR: TerminalWriteFailure — not signaling success for "
        ...
        print(
            f"Sending task success callback: request_id={request_id}, correlation_id={correlation_id}"
        )
        ...
        sfn_client.send_task_success(
            taskToken=task_token, output=json.dumps(callback_output, default=str)
        )
        ...
        error_message = str(e)
        print(
            f"ERROR processing request: request_id={request_id}, correlation_id={correlation_id}, error={error_message}"
        )
        ...
        try:
            sfn_client.send_task_failure(
                taskToken=task_token, error="BedrockProcessorError", cause=error_message
            )
