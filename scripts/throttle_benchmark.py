...
FEASIBLE = [
    ...
    ),  # LAST — burst disrupts this session's Opus quota
]
...
def big_prompt(approx_tokens):
    ...
    return ("Summarize in one word.\n" + unit * max(1, int(approx_tokens / 11)))[
        : approx_tokens * 4
    ]
...
def run_baseline(model_id, n, prompt, workers=200):
    ...
    with cf.ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        futs = [
            ex.submit(one_call, brt, model_id, f"u{i}-{i*7919}-{i*104729} " + prompt)
            for i in range(n)
        ]
...
def run_shaper(model_id, n, prompt, run_tag, drain_cap_s=720):
    ...
    with cf.ThreadPoolExecutor(max_workers=40) as ex:
        futs = [
            ex.submit(
                ...
                model_id,
                f"thr_{run_tag}_{i}",
                f"u{i}-{i*7919}-{i*104729} " + prompt,
            )
            for i in range(n)
        ]  # unique prefix: no cache hits
        for f in cf.as_completed(futs):
            r = f.result()
            if r:
                ids.append(r)
    ...
    while len(outcomes) < len(ids) and time.time() < deadline:
        ...
        for batch in chunks(missing, 100):
            ...
            for it in resp.get("Responses", {}).get(TABLE, []):
                ...
                outcomes[it["request_id"]] = {
                    "http": hs,
                    "ms": _iso_ms(it.get("created_at"), it.get("updated_at")),
                }
    ...
    for i in ids:
        if i in outcomes:
            ...
            res.append(
                {"http": 0, "ms": 0, "queued": True}
            )  # still draining (in-flight, not a failure)
    return res, time.time() - start
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=40000,
        help="approx input tokens/request (<60k for SFN 256KB)",
    )
    ap.add_argument(
        "--max-reqs",
        type=int,
        default=450,
        help="cap requests/model so big quotas drain in-window",
    )
    ...
    print(f"\n{'='*140}")
    print(
        f"MEANINGFUL-LOAD THROTTLE BENCHMARK | burst = {args.over}x real TPM quota | ~{args.prompt_tokens} in-tok/req "
    ...
    for mi, (alias, mid) in enumerate(models):
        ...
        if quota is None:
            print(
                f"▶ {alias}  ({mid})  SKIPPED — no usable TPM entry in .bedrock_quota_cache.json "
        ...
        n = min(
            args.max_reqs,
            max(4, int(round(quota * args.over / (args.prompt_tokens + 16)))),
        )
        offered = n * (args.prompt_tokens + 16)  # actual burst tokens
        print(
            f"▶ {alias}  ({mid})  real TPM quota={quota:,}  burst≈{offered:,.0f} tok ({offered/quota:.1f}x)  -> {n} reqs",
            flush=True,
        )
        try:
            ...
            print(
                f"    baseline: 200={b['success']} 429={b['throttle']} 503={b['rejected']} err={b['error']} "
                f"({b['throttle_pct']:.0f}% throttled) p50={b['p50']/1000:.1f}s p95={b['p95']/1000:.1f}s",
                flush=True,
            )
            ...
            s["cap"] = cap
            print(
                f"    shaper(cap={cap/1e6:.2f}M): 200={s['success']} 429={s['throttle']} 503={s['rejected']} "
    ...
    print(f"{'='*146}")
    h = (
        f"{'MODEL':15}{'quota':>7}{'burst':>8} | {'BASE 429%':>10}{'SHAP 429%':>10} | "
    ...
    for alias, quota, b, s in rows:
        print(
            f"{alias:15}{quota/1e6:>6.1f}M{b['offered']/1e6:>7.0f}M | "
            f"{b['throttle_pct']:>9.0f}%{s['throttle_pct']:>9.0f}% | "
            f"{b['success']:>9}{s['success']:>9}{s['rejected']:>9}{s['queued']:>8} | "
            f"{s2(b['p50'])+'/'+s2(b['p95']):>14}{s2(s['p50'])+'/'+s2(s['p95']):>16} | {s['cap']/1e6:>5.1f}M",
            flush=True,
        )
    print(f"{'='*146}")
    print(
        "BASE 429% = direct Bedrock at burst (heavy throttling). SHAP 429% = same load via shaper at its looped cap."
    )
    print(
        "shap 503 = shaper backpressure (queue full — clean reject, not a Bedrock 429). shap q = still draining at cap"
    )
    print(
        "(in-flight; SFN timeout 65min). Latency p50/p95 = end-to-end (shaper incl. queue wait), completed requests."
    )
    print(f"{'='*146}\n")
