...
def test_step_functions():
    ...
    if PROMPT_SIZE or MAX_TOKENS is not None:
        ...
        if tpm_burst > 0:
            ...
            print(
                f"TPM exhaustion: ~request #{int(tpm_exhaust)} | RPM exhaustion: ~request #{rpm_burst + 1}"
            )
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for i in range(NUM_REQUESTS):
            ...
            if delay_between_requests > 0:
                time.sleep(
                    delay_between_requests
                )  # nosemgrep: arbitrary-sleep -- intentional request-submission pacing in load test
                print(
    ...
    running_executions = {
        r["execution_arn"]: r for r in results if r["success"] and r["execution_arn"]
    }
    ...
    while (time.time() - wait_start) < max_wait_time:
        try:
            ...
            print(
                f"  Queue: {queue_depth}, Running: {running}, Succeeded: {final_succeeded + final_queued}, Failed: {final_failed}, Elapsed: {elapsed:.1f}s",
                end="\r",
            )
