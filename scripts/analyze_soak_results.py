...
def analyze_results_file(filepath):
    ...
    if checkpoints:
        ...
        print(
            f"  {'Time':>8s}  {'Sent':>7s}  {'OK':>7s}  {'Fail':>5s}  {'Queue':>5s}  {'DLQ':>4s}  {'Rate%':>6s}  {'p50ms':>6s}"
        )
        print(f"  {'─' * 60}")
        for cp in checkpoints:
            ...
            print(
                f"  {elapsed_h:7.1f}h  {cp['total_sent']:7d}  {ok:7d}  {cp['failed']:5d}  "
...
def analyze_cloudwatch(hours, region="us-east-1"):
    ...
    for metric_name in ["ConsumedWriteCapacityUnits", "ConsumedReadCapacityUnits"]:
        try:
            ...
            if datapoints:
                ...
                print(
                    f"    Trend: {'STABLE' if abs(last_hour - first_hour) / max(1, first_hour) < 0.2 else 'CHANGING'}"
                )
            else:
    ...
    for func_name, log_group_key in [
        ...
        try:
            ...
            for _ in range(30):
                ...
                time.sleep(
                    1
                )  # nosemgrep: arbitrary-sleep -- poll interval for CloudWatch Logs Insights query completion

            if result.get("results") and result["results"][0]:
                fields = {f["field"]: f["value"] for f in result["results"][0]}
                print(
                    f"  {func_name}: max={fields.get('max_mb', '?')}MB, avg={fields.get('avg_mb', '?')}MB, invocations={fields.get('invocations', '?')}"
                )
            else:
    ...
    try:
        ...
        if datapoints:
            ...
            if last > first * 1.5:
                print(
                    f"  WARNING: Item count growing ({first:.0f} -> {last:.0f}). TTL may not be cleaning up fast enough."
                )
            else:
