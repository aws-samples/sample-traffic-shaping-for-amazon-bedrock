...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="nova-2-lite",
        help="Model alias or ID (default: nova-2-lite)",
    )
    parser.add_argument(
        "--target-rpm",
        type=int,
        default=70,
        help="Target requests per minute (default: 70)",
    )
    parser.add_argument(
        "--duration-hours",
        type=float,
        default=1,
        help="Soak duration in hours (default: 1)",
    )
    parser.add_argument(
        "--adversarial-pct",
        type=float,
        default=5,
        help="Percentage of adversarial requests (default: 5)",
    )
    parser.add_argument(
        "--checkpoint-min",
        type=int,
        default=5,
        help="Minutes between checkpoint reports (default: 5)",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
        default=100,
        help="Default max_tokens per request (default: 100)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output JSON file for results (default: soak_results_<timestamp>.json)",
    )
    ...
    def snapshot(self):
        ...
        with self.lock:
            ...
            snap = {
                ...
                "success_rate": (self.total_succeeded + self.total_queued)
...
class SoakTest:
    ...
    def send_request(self, seq_num):
        ...
        try:
            result = self.sfn_client.start_execution(
                stateMachineArn=self.state_machine_arn, input=json.dumps(payload)
            )
            ...
            self.metrics.record_result(
                "failed", error_type=type(e).__name__, is_adversarial=is_adversarial
            )
                        ...
                        if status == "SUCCEEDED":
                            ...
                            try:
                                history = self.sfn_client.get_execution_history(
                                    executionArn=arn, maxResults=10, reverseOrder=True
                                )
                            ...
                            if error == "InputValidationError":
                                ...
                                self.metrics.record_result(
                                    "failed",
                                    error_type=error or status,
                                    is_adversarial=is_adv,
                                )
                        else:
                            self.metrics.record_result(
                                "failed", error_type=status, is_adversarial=is_adv
                            )
                except Exception:
    ...
    def print_checkpoint(self, snap):
        ...
        print(f"{'─' * 70}")
        print(
            f"  Sent: {snap['total_sent']} | Succeeded: {snap['succeeded']} | "
            f"Queued: {snap['queued']} | Failed: {snap['failed']} | Rejected: {snap['rejected']}"
        )
        print(
            f"  Success rate: {snap['success_rate']:.1f}% | Effective RPM: {snap['effective_rpm']:.1f}"
        )
        print(
            f"  Latency (5min window): p50={snap['latency_p50']:.0f}ms p95={snap['latency_p95']:.0f}ms p99={snap['latency_p99']:.0f}ms"
        )
        ...
        if snap["adversarial_sent"] > 0:
            print(
                f"  Adversarial: {snap['adversarial_sent']} sent, {snap['adversarial_rejected']} correctly rejected"
            )
        print(f"{'─' * 70}")
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"✓ Model config found: burst_capacity={config.get('burst_capacity')}, "
        ...
        while not self.shutdown.is_set():
            ...
            if seq_num % 10 == 0:
                ...
                m = self.metrics
                print(
                    f"  [{elapsed/60:.1f}m] Sent: {m.total_sent} | OK: {m.total_succeeded + m.total_queued} | "
        ...
        while self.pending and (time.time() - drain_start) < 300:
            ...
            time.sleep(
                2
            )  # nosemgrep: arbitrary-sleep -- intentional drain-polling interval in load test
