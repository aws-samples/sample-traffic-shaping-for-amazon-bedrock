...
def chart_1_success_rate_comparison():
    ...
    ax.set_title(
        "Request Success Rate — 150 Requests, 20 Workers, Jamba Model",
        fontsize=TITLE_SIZE,
        fontweight="bold",
        pad=15,
    )
...
def chart_2_burst_admission_before_after():
    ...
    bars1 = ax.bar(
        ...
        before,
        width,
        label="Pre-fix (Write-then-Verify)",
        color=COLORS["before"],
        edgecolor="white",
        linewidth=1.5,
    )
    bars2 = ax.bar(
        ...
        after,
        width,
        label="Post-fix (TransactWriteItems)",
        color=COLORS["after"],
        edgecolor="white",
        linewidth=1.5,
    )
    ...
    ax.set_title(
        "TransactWriteItems Fix Impact — 2000 Requests, burst_capacity=50",
        fontsize=TITLE_SIZE,
        fontweight="bold",
        pad=15,
    )
...
def chart_3_admission_breakdown():
    ...
    b3 = ax.bar(
        ...
        throttled,
        width,
        bottom=[b + q for b, q in zip(burst, queued_ok)],
    ...
    ax.set_ylabel("Request Count", fontsize=FONT_SIZE)
    ax.set_title(
        "Request Admission Breakdown Across Test Phases",
        fontsize=TITLE_SIZE,
        fontweight="bold",
        pad=15,
    )
...
def chart_4_queue_drain():
    ...
    ax.set_title(
        "Queue Drain — 1008 Items, 0 Failures (Phase 6c-stress-v2)",
        fontsize=TITLE_SIZE,
        fontweight="bold",
        pad=15,
    )
