...
def run_scenario(sc: Scenario, verbose: bool = False) -> dict:
    ...
    print(
        f"  → over target?  {'⚠️  YES' if over_target else '✅ no'}    "
        f"over quota (throttles)?  {'⚠️  YES' if over_quota else '✅ no'}"
    )

    return {
        "sc": sc,
        "actual_peak_60s": actual_peak_60s,
...
def main() -> None:
    ...
    print("=" * 76)
    print(
        f"  Deployed nova-2-lite config: queue_target_tpm={QUEUE_TARGET_TPM:,}, "
        f"tpm_queue_cap={TPM_QUEUE_CAP:,},"
    )
    print(
        f"  tpm_queue_regen={TPM_QUEUE_REGEN:,}/s, queue_capacity={QUEUE_CAPACITY:,}/min, "
